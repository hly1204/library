...
data:
  ...
  bundledCode: "#line 1 \"graph/maximum_flow_Dinic.hpp\"\n\n\n\n/**\n * @brief maximum\
    ...
    \ ++idx_[i.from], ++idx_[i.to];\n    for (int i = 0, sum = 0; i != n_ + 1; ++i)\
    \ sum += idx_[i], idx_[i] = sum - idx_[i];\n    for (auto &i : input_edge_) {\n\
    ...
    \ = level_[u] + 1;\n          q.push(edge_[i].to);\n        }\n    }\n    return\
    \ level_[t] != -1;\n  }\n\n  CapacityType augment(int from, CapacityType bottleneck,\
