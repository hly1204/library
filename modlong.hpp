...
class ModLong<Mod, true> {
    ...
    static std::enable_if_t<std::is_integral_v<Int>, ULL> safe_mod(Int v) {
    ...
    static ULL mul_high(ULL x, ULL y) {
        ULL a = x >> 32, b = x & -1U, c = y >> 32, d = y & -1U;
        ULL ad = a * d, bc = b * c;
        return a * c + (ad >> 32) + (bc >> 32) + (((ad & -1U) + (bc & -1U) + (b * d >> 32)) >> 32);
    }
...
public:
    ...
    friend bool operator!=(const ModLong &a, const ModLong &b) { return a.v_ != b.v_; }
    friend std::istream &operator>>(std::istream &a, ModLong &b) {
        LL v;
        a >> v;
