...
data:
  ...
  bundledCode: "#line 1 \"test/formal_power_series/polynomial_taylor_shift.1.test.cpp\"\
    ...
    \ std::vector<Tp> apply_egf(std::vector<Tp> F, std::vector<Tp> p) {\n    // kinda\
    \ transposed algorithm of binomial convolution.\n    const int n = p.size();\n\
    ...
    \ &b) { return a << b.val(); }\n};\n// clang-format on\n#line 8 \"test/formal_power_series/polynomial_taylor_shift.1.test.cpp\"\
    \n\ntemplate<typename Tp>\nstd::vector<Tp> taylor_shift_via_linear_operator(const\
    \ std::vector<Tp> &a, Tp c) {\n    const int n = a.size();\n    auto &&bin  =\
    \ Binomial<Tp>::get(n);\n    std::vector<Tp> f(n);\n    Tp cc = 1;\n    for (int\
    \ i = 0; i < n; ++i) f[i] = cc * bin.inv_factorial(i), cc *= c;\n    return apply_fps(f,\
  ...
  code: "#define PROBLEM \"https://judge.yosupo.jp/problem/polynomial_taylor_shift\"\
    ...
    \n#include <iostream>\n#include <vector>\n\ntemplate<typename Tp>\nstd::vector<Tp>\
    \ taylor_shift_via_linear_operator(const std::vector<Tp> &a, Tp c) {\n    const\
    \ int n = a.size();\n    auto &&bin  = Binomial<Tp>::get(n);\n    std::vector<Tp>\
    \ f(n);\n    Tp cc = 1;\n    for (int i = 0; i < n; ++i) f[i] = cc * bin.inv_factorial(i),\
    \ cc *= c;\n    return apply_fps(f, a);\n}\n\nint main() {\n    std::ios::sync_with_stdio(false);\n\
    \    std::cin.tie(nullptr);\n    using mint = ModInt<998244353>;\n    int n;\n\
    \    mint c;\n    std::cin >> n >> c;\n    std::vector<mint> a(n);\n    for (int\
    \ i = 0; i < n; ++i) std::cin >> a[i];\n    const auto res = taylor_shift_via_linear_operator(a,\
    \ c);\n    for (int i = 0; i < n; ++i) std::cout << res[i] << ' ';\n    return\
    \ 0;\n}\n"
  dependsOn:
  - binomial.hpp
  ...
  verifiedWith: []
documentation_of: test/formal_power_series/polynomial_taylor_shift.1.test.cpp
layout: document
redirect_from:
- /verify/test/formal_power_series/polynomial_taylor_shift.1.test.cpp
- /verify/test/formal_power_series/polynomial_taylor_shift.1.test.cpp.html
