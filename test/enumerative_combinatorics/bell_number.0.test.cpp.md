...
data:
  ...
  bundledCode: "#line 1 \"test/enumerative_combinatorics/bell_number.0.test.cpp\"\n\
    ...
    \ f(g) mod x^n\n// see:\n// [1]: Yasunori Kinoshita, Baitian Li. Power Series\
    ...
    \ Tp> inline std::vector<Tp> polya_q(std::vector<Tp> a, int n) {\n    if (n <=\
    \ 0) return {};\n    a.resize(n);\n    assert(a[0] == 0);\n    a[0] = 1;\n   \
    ...
    template<typename Tp> inline std::vector<Tp> polya_exp_m(std::vector<Tp> a, int\
    \ n) {\n    if (n <= 0) return {};\n    a.resize(n);\n    assert(a[0] == 0);\n\
    \    auto &&bin = Binomial<Tp>::get(n);\n    for (int i = n - 1; i > 0; --i)\n\
    \        for (int j = 2; i * j < n; ++j)\n            if (j & 1) {\n         \
    \       a[i * j] += a[i] * bin.inv(j);\n            } else {\n               \
    ...
    \ 10 \"poly_basic.hpp\"\n\ntemplate<typename Tp> inline int degree(const std::vector<Tp>\
    \ &a) {\n    int n = (int)a.size() - 1;\n    while (n >= 0 && a[n] == 0) --n;\n\
    \    return n;\n}\n\ntemplate<typename Tp> inline void shrink(std::vector<Tp>\
    ...
    \ Tp> inline std::vector<Tp> pow_table(int e, int n) {\n    if (n <= 0) return\
    ...
    \ number\ntemplate<typename Tp> inline std::vector<Tp> partition_function(int\
    \ n) {\n    assert(n >= 0);\n    std::vector<Tp> I(n);\n    for (int i = 1; i\
    \ < n; ++i) I[i] = 1;\n    return polya_exp(I, n);\n}\n\n// returns |s(n,0)|,\
    \ ..., |s(n,n)|\n// unsigned Stirling numbers of the first kind\ntemplate<typename\
    \ Tp> inline std::vector<Tp> unsigned_stirling_numbers_1st_row(int n) {\n    assert(n\
    \ >= 0);\n    if (n == 0) return {Tp(1)};\n    int mask = 1 << 30;\n    while\
    \ ((mask & n) == 0) mask >>= 1;\n    std::vector<Tp> res{Tp(0), Tp(1)};\n    for\
    \ (int d = 1; d != n;) {\n        res = convolution(res, taylor_shift(res, Tp(d)));\n\
    \        d <<= 1;\n        if ((mask >>= 1) & n) {\n            res = convolution(res,\
    \ std::vector<Tp>{Tp(d), Tp(1)});\n            d |= 1;\n        }\n    }\n   \
    ...
    \ + 1);\n    return res;\n}\n\n// returns S(0,k), ..., S(n-1,k)\ntemplate<typename\
    \ Tp> inline std::vector<Tp> stirling_numbers_2nd_column(int k, int n) {\n   \
    ...
    \ A(n,0), ..., A(n,n)\ntemplate<typename Tp> inline std::vector<Tp> eulerian_numbers_row(int\
    ...
    template<typename Tp> inline std::vector<Tp> eulerian_numbers_column(int k, int\
    ...
    \ Int>\n    static std::enable_if_t<std::is_integral_v<Int>, unsigned> safe_mod(Int\
    \ v) { using D = std::common_type_t<Int, unsigned>; return (v %= (int)Mod) < 0\
    ...
    \ &b) { return a << b.val(); }\n};\n// clang-format on\n#line 6 \"test/enumerative_combinatorics/bell_number.0.test.cpp\"\
    \n\nint main() {\n    std::ios::sync_with_stdio(false);\n    std::cin.tie(nullptr);\n\
    \    using mint = ModInt<998244353>;\n    int n;\n    std::cin >> n;\n    const\
    \ auto B = bell_numbers<mint>(n + 1);\n    for (int i = 0; i <= n; ++i) std::cout\
    \ << B[i] << ' ';\n    return 0;\n}\n"
...
layout: document
redirect_from:
- /verify/test/enumerative_combinatorics/bell_number.0.test.cpp
- /verify/test/enumerative_combinatorics/bell_number.0.test.cpp.html
