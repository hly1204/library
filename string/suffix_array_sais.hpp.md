...
data:
  ...
  code: "#ifndef SUFFIX_ARRAY_SAIS_HEADER_HPP\r\n#define SUFFIX_ARRAY_SAIS_HEADER_HPP\r\
    ...
    \ (!t[x])\r\n#define IS_LMS_TYPE(x) (IS_S_TYPE(x) && x != 0 && IS_L_TYPE(x - 1))\r\
    \n  t.back() = true;\r\n  for (int i = n - 2; i >= 0; --i) {\r\n    t[i] = (s[i]\
    \ < s[i + 1] || (IS_S_TYPE(i + 1) && s[i] == s[i + 1]));\r\n    if (IS_LMS_TYPE(i\
