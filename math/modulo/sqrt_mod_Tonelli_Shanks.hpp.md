...
data:
  ...
  attributes:
    document_title: "square root mod Tonelli-Shanks / \u6A21\u610F\u4E49\u4E0B\u5E73\
      \u65B9\u6839 Tonelli-Shanks \u7B97\u6CD5"
    links: []
  bundledCode: "#line 1 \"math/modulo/sqrt_mod_Tonelli_Shanks.hpp\"\n\n\n\n/**\n *\
    \ @brief square root mod Tonelli-Shanks / \u6A21\u610F\u4E49\u4E0B\u5E73\u65B9\
    \u6839 Tonelli-Shanks \u7B97\u6CD5\n *\n */\n\n#include <cassert>\n#include <optional>\n\
    ...
    \ namespace lib\n\n\n#line 15 \"math/modulo/sqrt_mod_Tonelli_Shanks.hpp\"\n\n\
    namespace lib {\n\n/**\n * @brief \u6A21\u5E73\u65B9\u6839\n * @tparam T\n * @param\
    \ x [0, p-1] \u4E2D\u7684\u4E00\u4E2A\u503C\n * @param p \u7D20\u6570\n * @return\
    \ std::optional<T> \u8FD4\u56DE\u8F83\u5C0F\u7684\u4E00\u4E2A\uFF0C\u82E5 x=0\
    \ \u5219\u8FD4\u56DE 0 \uFF0C\u65E0\u89E3\u5219\u8FD4\u56DE\u7A7A\n */\ntemplate\
    \ <typename T> std::optional<T> sqrt_mod_prime(T x, T p) {\n  if (p == 2 || x\
    \ <= 1) return x;\n  if (pow_mod(x, p >> 1, p) != 1) return {}; // (p>>1)=(p-1)/2\
    \ \u6B27\u62C9\u51C6\u5219\n  if ((p & 3) == 3) {\n    x = pow_mod(x, (p + 1)\
    \ >> 2, p);\n    return x < p - x ? x : p - x;\n  }\n\n  T s = p - 1;\n  int r\
    ...
    \    if (b >= pe) b -= pe;\n  }\n  return inv_mod(b, pe);\n}\n\n} // namespace\
    \ lib\n\n\n"
  code: "#ifndef SQRT_MOD_TONELLI_SHANKS_HEADER_HPP\n#define SQRT_MOD_TONELLI_SHANKS_HEADER_HPP\n\
    \n/**\n * @brief square root mod Tonelli-Shanks / \u6A21\u610F\u4E49\u4E0B\u5E73\
    \u65B9\u6839 Tonelli-Shanks \u7B97\u6CD5\n *\n */\n\n#include <cassert>\n#include\
    \ <optional>\n#include <random>\n\n#include \"../basic/binary_pow.hpp\"\n#include\
    \ \"../basic/exgcd.hpp\"\n\nnamespace lib {\n\n/**\n * @brief \u6A21\u5E73\u65B9\
    \u6839\n * @tparam T\n * @param x [0, p-1] \u4E2D\u7684\u4E00\u4E2A\u503C\n *\
    \ @param p \u7D20\u6570\n * @return std::optional<T> \u8FD4\u56DE\u8F83\u5C0F\u7684\
    \u4E00\u4E2A\uFF0C\u82E5 x=0 \u5219\u8FD4\u56DE 0 \uFF0C\u65E0\u89E3\u5219\u8FD4\
    \u56DE\u7A7A\n */\ntemplate <typename T> std::optional<T> sqrt_mod_prime(T x,\
    \ T p) {\n  if (p == 2 || x <= 1) return x;\n  if (pow_mod(x, p >> 1, p) != 1)\
    \ return {}; // (p>>1)=(p-1)/2 \u6B27\u62C9\u51C6\u5219\n  if ((p & 3) == 3) {\n\
    \    x = pow_mod(x, (p + 1) >> 2, p);\n    return x < p - x ? x : p - x;\n  }\n\
    \n  T s = p - 1;\n  int r = 0;\n  while ((s & 1) == 0) {\n    ++r;\n    s >>=\
    ...
    \ {\n    }\n  }\n  return x < p - x ? x : p - x;\n}\n\n/**\n * @brief Hensel lifting\
    \ \u6C42\u6A21\u7D20\u6570\u5E42\u6B21\u7684\u5E73\u65B9\u6839\uFF0C\n * @tparam\
    \ T\n * @param x [0, p^e-1] \u4E2D\u7684\u4E00\u4E2A\u503C\u4E14 x \u4E0D\u80FD\
    \u6574\u9664\u6A21\u6570\n * @param p \u6A21\u6570\u4E0D\u80FD\u4E3A 2\n * @param\
...
documentation_of: math/modulo/sqrt_mod_Tonelli_Shanks.hpp
layout: document
