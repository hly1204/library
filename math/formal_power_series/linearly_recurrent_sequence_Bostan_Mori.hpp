...
public:
  ...
  mod_t operator[](unsigned long long n) const {
    ...
    for (;; n >>= 1) {                                         // p(x)/q(x) = p(x)q(-x)/(q(x)q(-x))
      ...
      NTT<mod_t>::get_instance().even_dft(len, q_cpy.data());
      ...
      NTT<mod_t>::get_instance().dft_doubling(len >> 1, q_cpy.data());
      NTT<mod_t>::get_instance().dft_doubling(len >> 1, p_cpy.data());
    }
  }
