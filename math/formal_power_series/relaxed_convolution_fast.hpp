#ifndef RELAXED_CONVOLUTION_FAST_HEADER_HPP
#define RELAXED_CONVOLUTION_FAST_HEADER_HPP

/**
 * @brief relaxed convolution fast
 *
 */
...
class RelaxedConvolutionFast {
public:
  RelaxedConvolutionFast(const std::vector<mod_t> &A, const std::vector<mod_t> &B)
      : A_(A), B_(B), contribution_(4), n_(0) {
    contribution_.reserve(1024);
  }
  ~RelaxedConvolutionFast() = default;
